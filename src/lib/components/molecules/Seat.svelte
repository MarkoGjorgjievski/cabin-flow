<script>
  ...
  import { badgeTextColor } from "$system/badge";
...
<div class={clsx(aisleAfterSeat.includes(seat.at(-1)) && 'pr-4')}>
  <div class={clsx([seatType.get(cabin)], ghostSeats.includes(seat) && 'invisible', 'aircraft-seat relative border-base-300', passenger ? 'text-info/80' : 'border border-gray-600 text-gray-600 border-dashed bg-transparent')}>
    {seat}
